...
export interface ParsedLessonImage {
  filename: string;
  mimeType: string;
  size: number;
  stream: PassThrough;
...
export function parseLessonForm(req: Request): Promise<ParsedLessonForm> {
  return new Promise((resolve, reject) => {
    ...
    const busboy = Busboy({
      headers: req.headers,
      limits: { files: 1, fileSize: MAX_IMAGE_SIZE, fields: 12 },
    });

    const fields: Record<string, string> = {};
    let thumbnail: ParsedLessonImage | undefined;
    let settled = false;
    ...
    busboy.on('finish', () => {
      ...
      if (!name) {
        fail(new AppError('Lesson name is required', ERROR_CODES.VALIDATION_ERROR, 400));
        return;
      }
      ...
      const moduleId = fields.moduleId?.trim();
      if (!moduleId) {
        fail(new AppError('Module is required', ERROR_CODES.VALIDATION_ERROR, 400));
        return;
      }
