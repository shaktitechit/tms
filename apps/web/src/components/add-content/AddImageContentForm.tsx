...
import { getErrorMessage, useCreateImageMutation } from '@/store/api';
import { ContentFormActions } from './ContentFormActions';
import { fileInputClassName, type LessonContentFormProps } from './types';

export function AddImageContentForm({ lessonId, onCancel, onSuccess }: LessonContentFormProps) {
  ...
  async function onSubmit(event: React.FormEvent) {
    ...
    try {
      ...
      setError(getErrorMessage(err, 'Could not add image'));
    }
  }
      ...
      {file && previewUrl ? (
        <div className="space-y-3 rounded-2xl border border-blue-100 bg-white p-4">
          ...
          <img
            src={previewUrl}
            alt={title || file.name}
            className="max-h-64 w-full rounded-xl object-contain bg-slate-50"
          />
