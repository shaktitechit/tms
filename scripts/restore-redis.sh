#!/usr/bin/env bash
# Restore Redis from a backup created by scripts/backup-redis.sh.
# Usage: scripts/restore-redis.sh backups/redis-YYYYMMDD-HHMMSS.tar.gz
set -euo pipefail
cd "$(dirname "$0")/.."
...
if [[ -z "$ARCHIVE" ]]; then
  echo "Usage: scripts/restore-redis.sh <backup.tar.gz>" >&2
  echo "Available backups:" >&2
  ls -1 backups/redis-*.tar.gz 2>/dev/null || echo "  (none in ./backups)" >&2
  exit 1
fi
...
if [[ -d "$DATA_DIR" ]]; then
  PREV="backups/redis-data-replaced-$(date +%Y%m%d-%H%M%S)"
  mkdir -p backups
